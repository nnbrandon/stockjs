...
export function getStoredSymbols() {
  return db[STORE].orderBy("symbol").uniqueKeys();
}

export async function getStoredSymbolsWithNames() {
  const symbols = await db[STORE].orderBy("symbol").uniqueKeys();
  return Promise.all(
    symbols.map(async (symbol) => {
      const record = await db[STORE].where("symbol").equals(symbol).first();
